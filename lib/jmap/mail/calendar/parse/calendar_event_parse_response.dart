...
class CalendarEventParseResponse extends ParseResponse<List<CalendarEvent>> {
  CalendarEventParseResponse(AccountId accountId,
      {Map<Id, List<CalendarEvent>>? parsed,
      List<Id>? notParsable,
      List<Id>? notFound})
      : super(accountId,
            parsed: parsed, notParsable: notParsable, notFound: notFound);

  static CalendarEventParseResponse deserialize(Map<String, dynamic> json) {
    return CalendarEventParseResponse(
        const AccountIdConverter().fromJson(json['accountId'] as String),
        parsed: (json['parsed'] as Map<String, dynamic>?)?.map((key, value) =>
            MapEntry(
